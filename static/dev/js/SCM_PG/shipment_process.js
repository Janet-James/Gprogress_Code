...
$(document).ready(function() {
    ...
    global_shipment_scm_type=$('#shipment_scm_type').val();
    shipment_data_load()
});
...
function shipment_data_load(){
    var csrf_data = $("input[name=csrfmiddlewaretoken]").val();
            $.ajax({
                ...
                type: 'POST',
                async: false, 
                data: { 'shipment_item_id': global_shipment_item_id, 'shipment_scm_type': global_shipment_scm_type},
                ...
                console.log("shipment_product_data",shipment_product_data)
    ...
    $('#generate_packing_label').on('click', function () {
        ...
        var csrf_data = $("input[name=csrfmiddlewaretoken]").val();
            $.ajax({
                ...
                type: 'POST',
                async: false, 
                data: { 'shipment_item_id': global_shipment_item_id,'shipment_scm_type': global_shipment_scm_type,'packing_data':JSON.stringify(tableData),"box_item_data":JSON.stringify(boxdata)},
                csrfmiddlewaretoken: csrf_data
            }).done(function(json_data) {
                data=JSON.parse(json_data)
                console.log(data)
        ...
        if ($(this).is(':checked')) {
            ...
            $.each(packing_data, function(index, product) {
                ...
                if (existing_product.length > 0) {
                    ...
                    combined_boxes_data[box_no].push({ 'name': product_name, 'quantity': quantity ,'hsn_code': hsn_code});
                }
            });
            ...
            $.each(combined_boxes_data, function(boxNumber, products) {
                ...
                $.each(products, function(index, product) {
                    ...
                    dataDesc.append('<div class="subData hsn_code hidden">' + product.hsn_code + '</div>');
...
function GetBoxItemData()
{
    ...
    $('.dataItem').each(function () {
        ...
        $(this).find('.dataDesc').each(function () {
            ...
            var hsn_code = $(this).find('.hsn_code').text().trim();
