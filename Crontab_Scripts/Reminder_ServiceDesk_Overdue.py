...
def service_team():
    ...
    for data in ticket_data:
        ...
        # Splitting reported date and time
        date_part, time_part = reported_date_time.split('T')
        reported_date = datetime.strptime(date_part, '%Y-%m-%d').date()  
        reported_time = datetime.strptime(time_part[:-6], '%H:%M:%S')
        time_format = reported_time.strftime('%I:%M %p')
        date_format = reported_date.strftime('%d-%b-%Y')
     
        # Get current date only
        current_date = datetime.now().date()

        # day difference
        date_difference = current_date - reported_date
        day_count = date_difference.days
        days = str(day_count)
        if days == '1':
...
def send_email(html_body, responsible_mails, cc_mails, external_emails, days_difference, service_call_no):
    ...
    to_address = ['glen@xaasability.com','arun@xaasability.com','veerabagu@xaasability.com','suthan@xaasability.com']
    cc_address = ['bernard@green.com.pg','juniorsolutiondeveloper2@greenmaestros.co.in','janet.james@nexttechnosolutions.co.in']
    bcc_address = ['solutiondeveloper2@greenmaestros.co.in', 'sandhiya.arjunan@nexttechnosolutions.co.in', "vijith.vijayan@nexttechnosolutions.co.in"]
