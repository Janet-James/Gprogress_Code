        ...
        for resp_id in receipient_property.values():
            ...
            finac_pg_voucher_params = {
                ...
                "filter": {
                    ...
                    "stageId": ["DT144_12:NEW", "DT144_12:UC_C61M6D"],
                    "categoryId": 12
                }
            }
            vouchers_list = bx24.get_all('crm.item.list', finac_pg_voucher_params)
            if not vouchers_list:
                print(f"No Voucher are Receipient by {first_name + ' ' + last_name}")
            else:
                for voucher in vouchers_list:
                    item_id = voucher.get('id')
                    supplier_attach_receipt = voucher.get("ufCrm8_1680165669354")
                    if supplier_attach_receipt:
                        print(" -- Already Have the Supplier Payment Receipt -- ", item_id)
                    else:
                        if voucher.get("stageId") == "DT144_12:UC_C61M6D":
                            print("-- BANK RECONCILATION STAGE -- ")
                        else:
                            print("-- SUPPLIER PAYMENT RECEIPT & TAX INVOICE STAGE -- ")
                ...
                <p style="font-family: sans-serif; font-size: 14px; margin-top: 20px;margin-bottom: 20px;">Please ensure the supplier payment receipt is attached to the respective <a href="https://greenltd.bitrix24.com/crm/type/144/kanban/category/12/">voucher(s)</a> at earliest.</p>
                ...
                subject = salutation + full_name + ' - Reminder to Attach Suppiler Payment Receipt'+" ("+current_date+")"
