...
for uni_list in mail_recipient_response:
    ...
    for persons_id in uni_list_recipient_persons.values():
        ...
        params = {
         "entityTypeId": 163,
         "filter":{"createdBy":recipient_user_id, "stageId":"DT163_116:PREPARATION", "categoryId":116}
        }
        ...
        if not scm_pg_response:
            ...
            for scm_in_item in scm_pg_response:
                item_id = scm_in_item.get('id')
                voucher_title = scm_in_item.get('title')
                my_company_id = str(scm_in_item.get('mycompanyId'))
                supplier = scm_in_item.get('ufCrm40_1678433789')
                amount = scm_in_item.get('opportunity')
                currencyid = scm_in_item.get('currencyId')
                finacial_year = str(scm_in_item.get('ufCrm40_1681103293165'))
                if finacial_year=="446":
                    financial_year_value='2022-2023'
                if finacial_year=="448": 
                    financial_year_value='2023-2024'
                    
                created_date_str = scm_in_item.get("createdTime")
                ...
                finacial_year_name = financial_year_value
                ...
                <td style="color: #008F2F;"><a href="https://greenltd.bitrix24.com/crm/type/163/details/{item_id}/"><span>View Voucher</span></a></td>
                </tr>"""
                count += 1
            # Email content
            ...
            <p style="font-family: sans-serif; font-size: 14px; margin-top: 20px;margin-bottom: 20px;">Some of your <a href="https://greenltd.bitrix24.com/crm/type/163/details/">Vouchers</a> are still in the "Review" state. Please ensure that you contact the respective approver to progress the Voucher to the next stage.</p>
