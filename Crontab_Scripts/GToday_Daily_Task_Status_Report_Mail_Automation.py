    ...
    if date in comment_data_dict:
        ...
        for comment, task_id,task_title,comment_created_date in comments_on_date:
            ...
            for title in task_title:
                ...
                status_and_deadline_dict[task_id] = {'status_name': status_name, 'formatted_deadline': formatted_deadline}    
    ...
    for user_detail in user_details:
        ...
        if date in comment_data_dict:
            ...
            if date == comparative_date_format:
                ...
                for i in final_data:
                    final_task_id = i[1]
                    print("STATUSSSS------------", status_and_deadline_dict)
                    if final_task_id in status_and_deadline_dict:
                        status_info = status_and_deadline_dict[final_task_id]
                        final_status_name = status_info['status_name']
                        final_formatted_deadline = status_info['formatted_deadline']

                        print(f"-------------------------- JJJJJ {final_task_id} - Status: {final_status_name}, Deadline: {formatted_deadline}")
                        table_2 += f"""
                            ...
                            <td style="text-align: center;">{final_formatted_deadline}</td>
                            <td style="text-align: center;">{i[0]}</td>
                            <td style="text-align: center;">{final_status_name}</td>
                            </tr>"""
                        count += 1
            ...
            mail_content1 = f"""<body style="font-family: sans-serif;">
                ...
                <h3> You Haven't Added Task Summary Today ({current_date.strftime('%d/%m/%Y')})</h3>
                """
            ...
            bcc_address = ["janet.james@nexttechnosolutions.co.in"]
            to_address = user_email
            subject = f"{salutation}{user_name} {user_last_name},Your Daily Execution Status For {current_date.strftime('%d/%m/%Y')}, Week #{current_week_number}"
            message = MIMEMultipart()
            message['From'] = "GREEN GTODAY"
            message['To'] = to_address
            message['Cc'] = ",".join(cc_address)
